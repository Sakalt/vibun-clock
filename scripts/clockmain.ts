...
export class EikyuDate {
    ...
    toMiliseconds(yea: number = 1240, mon: number = 1, day: number = 1, hou: number = 0, per: number = 0, min: number = 0, sec: number = 0, mil: number = 0) {
    ...
    getFunScripted() {
        ...
        return `${obj.fyea}年${obj.mon}月${obj.day}日 ${obj.fweek}曜日 ${obj.hou}時${obj.per}刻${obj.min}分${obj.sec}秒`
    }
}